...
- **Watcher: ground-truth capture for tools that can't drive MCP** — for every other case (browser-only agents like v0/Bolt, raw `git commit`s, autonomous CI bots, agents that haven't adopted MCP yet, sessions where the agent forgot to annotate), the silent-capture watcher observes what changed via git + filesystem + 6-layer UBA attribution. It guarantees the *what* is recorded even when no MCP call ever fires — and Phase 2's async LLM synthesis worker (see [docs/Business/execution-plan/23-catch-up-implementation-plan-2026q3.md](../../docs/Business/execution-plan/23-catch-up-implementation-plan-2026q3.md)) fills in the *why* from diff + chat transcript when the agent didn't.
